...
graph TD
    ...
    subgraph Backend: bridgePlugin в Vite
        ...
        Capture[Capture: запись в .pfdrec]
    ...
    %% Потоки Replay
    API -->|Списки и файлы .pfdrec| Viewer
...
2. В папке `captures/` создается файл `.pfdrec` с именем вида `{источник}_{длительность}.pfdrec`.
...
1. **Запись**:
   ...
   - Бэкенд начинает рассчитывать физику и одновременно с этим пишет каждый сгенерированный кадр `TelemetryFrame` в `.pfdrec` файл в папку `captures/`.
...
4. **Интеграция Replay в PFD**:
   - Добавление контрольной панели плеера (шкала времени, кнопки воспроизведения) непосредственно на страницу PFD для удобного проигрывания записанных `.pfdrec` треков без перехода в Diagnostic Viewer.
