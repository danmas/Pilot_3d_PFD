/**
 * Единый каталог всех телеметрических параметров (132 поля).
 *
 * Порядок полей СТРОГО соответствует порядку слотов в out.json —
 * это определяет байтовую раскладку бинарного потока.
 *
 * Из этого файла автогенерируется:
 *   - UdpServerConfig.json (Parameters)
 *   - AVIONICS_FIELDS (подмножество для PFD)
